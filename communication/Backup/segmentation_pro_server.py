...
class FrameCap(mp.Process):       # defining a thread class
	...
	#Decision array is to determine keeping the frame or dropping it,
        ...
        i    = i.append(1)				 #Adding the first frame index in the begining of the of the Decision array
        return i                                         #reading order is from the last index is the begining (speed issue)
    ...
    def run(self):
        try :
            ...
            while (self.key.value and success):
                if i.pop():		        # Taking a decision to drop or concatente it onto the frames name of the class "FrameCap"
